...
import AppNavBar from '../components/navbarAgent'; // Replace with the correct path
...
function TicketsTable() {
  ...
  const handlePriorityChange = (ticketId, newPriority) => {
    ...
    axios.put(`http://localhost:3000/api/v1/agents/changeTicketPriority/${ticketId}/${newPriority}`, {
        withCredentials: true,
        headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  const handleCloseTicket = (ticketId, agentResponse) => {
    ...
    axios.put(`http://localhost:3000/api/v1/agents/closeTicket/${ticketId}`, { agentResponse }, { ...bearerHeader }, {
        withCredentials: true,
        headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
